...
class Kladr extends Model
{
    ...
    public static function city($city_name, $region_code, $limit = null, $with_socr = true) {
        ...
        $data = DB::select("SELECT `CODE`, `SOCR`, `NAME` 
        FROM kladr.kladr 
        WHERE $where 
        ...
        if (!empty($data)) {
            foreach ($data as $item) {
                $result['results'][] = [
                    'id' => $item->CODE,
                    'text' => ($with_socr && !empty($item->SOCR) ? $item->SOCR . '. '  : '') . $item->NAME
                ];
            }
        }
    ...
    public static function street($street_name, $city_code = null, $region_code = null, $limit = null, $with_city_title = false) {
        ...
        SELECT street.`CODE`, street.`SOCR`, city.`SOCR` `CITY_SOCR`, " . ($with_city_title ? "CONCAT_WS(', ', street.`NAME`, city.`NAME`) `NAME`" : 'street.`NAME`') . "
